      ...
      <div class="container">
         <div class="row">
            <div class="col s10 m6 l6">
               <h5 class="breadcrumbs-title mt-0 mb-0"><span>Production Analysis </span></h5>
   ...
   <div class="col s12">
      <div class="container">
         <div class="section">
            <div class="card">
               <div class="card-content">
                  <p class="caption mb-0">
                    Hello {{ Session::get('USERNAME') }}, here's Production Analysis form. Have a good day :)
                  </p>
               </div>
            </div>
                ...
                <div class="card-content">
                    ...
                    <div class="row">
                        <div class="input-field col m4 s12">
                            ...
                            <select class="browser-default" id="mach_id" name="mach_id" >
                            <option></option>
                            @if(isset($listmachine))
                                @foreach($listmachine as $s)
                                    <option value='{{ $s->mach_id }}'>{{ $s->descr}}</option>
                                @endforeach
                            @endif
                            </select>
                            <label>Machine</label>
            ...
            <div class="card" id="sumProductAnalysis" style="display: none">
                <div class="card-content">
            ...
            <div class="card" id="sumProductAnalysisDetail" style="display: none">
                <div class="card-content">
                    <div class="section users-view">
                        <div class="row indigo lighten-5">
                        <div class="col s12 m12 users-view-timeline">
                            <h6 class="indigo-text m-0">Query Result Detail: </h6>
                        </div>
                        </div>
                        <br>
                        <div class="row">
...
var dt, container, title, label;
var dt_detail, container_detail, title_detail, label_detail;
var x = window.matchMedia("(max-width: 991px)");
var Qmach_id, Qstart, Qend, Qbrand, Qcoat, Qgrade, Qthick, Qwidth, Qcolour, Qquality;
...
function getChart(dt, container, title, label){
...
function onClick(e){
    ...
    $.ajax({
        type: "POST",
        url: "{{ url('getProductAnalysisDetail') }}",
        data: {
            '_token': '{{ csrf_token() }}',
            'mach_id': Qmach_id,
            'periode' : param,
        ...
        success: function (data) {
            ...
            sumProductAnalysisDetail.style.display = "block";
            ...
            if (data['qGrade'].length > 0) {
                ...
                for (var i = 0; i < data['qGrade'].length; i++) {
                    ...
                    for (var i = 0; i < data['qGrade'].length; i++) {
                        ...
                        dt_detail.push({  label: data['qGrade'][i].grade_name,  y: parseInt(data['qGrade'][i].total_wgt), legendText: data['qGrade'][i].grade_name });
            ...
            if (data['qColor'].length > 0) {
                ...
                for (var i = 0; i < data['qColor'].length; i++) {
                    ...
                    for (var i = 0; i < data['qColor'].length; i++) {
                        ...
                        dt_detail.push({  label: data['qColor'][i].color_desc,  y: parseInt(data['qColor'][i].total_wgt), legendText: data['qColor'][i].color_desc });
            ...
            $('html, body').animate({
                scrollTop: $("#sumProductAnalysisDetail").offset().top
            }, 1200)
...
$(document).ready(function() {
    ...
    var sumProductAnalysis = document.getElementById("sumProductAnalysis")
    var sumProductAnalysisDetail = document.getElementById("sumProductAnalysisDetail")

    $('#SunriseSystem').addClass('active open');
    $('#SunriseSystemCSS').css('display','block');
    $('#ProductionAnalysis').addClass('active gradient-45deg-green-teal gradient-shadow');

    listBrand();listCoat();
    listGrade();allThickness();allWidth();allColour();allQuality();

    $("#mach_id").select2({
        placeholder: "Choose Machine first"
    });
    ...
    $('#reset').on('click', function() {
        ...
        f1.clear();
        f2.clear();
        sumProductAnalysis.style.display = "none";
        sumProductAnalysisDetail.style.display = "none";
    ...
    $('#go').on('click', function() {
        ...
        label = '';
        Qmach_id = $('#mach_id').val();
        if (Qmach_id) {

            label = label+'MachID: '+Qmach_id.trim();
        ...
        if (!Qmach_id) {
            ...
            swal("Whops", "Choose machine first", "error");
        }
                ...
                success: function (data) {
                    ...
                    if (data.length > 0) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            dt.push({ label: data[i].periode , y: parseFloat(data[i].total_wgt)});
                        }
                        container = 'container1';
                        title = "CGL"+Qmach_id+" Summary";
                        sumProductAnalysis.style.display = "block";
                        sumProductAnalysisDetail.style.display = "none";
                        getChart(dt, container, title, label);
                        $.unblockUI();

                        $('html, body').animate({
                            scrollTop: $("#sumProductAnalysis").offset().top
                        }, 1200)
                    }
